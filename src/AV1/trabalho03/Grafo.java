...
public class Grafo<T> {
    ...
    public void naoEuleriano() {
        int contador = 0;
        for (Map.Entry<T, List<T>> indice : tabelaHash.entrySet()) {
            if (indice.getValue().size() % 2 != 0) {
                contador++;
            }
        }
        if (contador >= 2) {
    ...
    public void semiEuleriano() {
        ...
        if (contador == 2) {
            System.out.println("Este grafo é semi-euleriano.");
        }
    }
    ...
    public void euleriano() {
        ...
        if (par) {
            System.out.println("Este grafo é euleriano.");
        }
    }
